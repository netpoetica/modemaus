$(document).ready(function () {
	...
	var BuildScale = {
		...
		Build: function(root, mode){
			...
			//represents the addition of each modes whole or half step
			var currentDegree = root, finalScale = [], i=0, j=mode.steps.length;
			for(i; i<j; i+=1){
	...
	Fretboard = {
		...
		Draw: function(tuning){
			...
			var tableString = "<tr>", n=0, i = 0, ii = 6,j = 0, jj = tuning.chromaticNumbering[i];
			for (n; n < 12; n+=1){		//12 is number of guitar frets shown, 0 = open
				tableString += "<th>" + n + "</th>";
			}
			tableString += "</tr>";
			//table body
			for(i; i < ii; i+=1){	//6 is the number of guitar strings
				tableString += "<tr id='string" + i + "'>";
				//start at the first note of the tuning, and work your way to the last (0-5) for 6 strings
				for(j; j < 12; j+=1){	//12 is number of frets shown, using Standard tuning, + j 
	...
	showOutput = function(rootNote, scaleName, scale, chorder){
		...
		$("#contentHeader").html(
			'<h2>' + rootNote + ' ' + scaleName + '</h2>'	
		);
		
		//reference to the output Panel
		var output = $("#outputPanel"), html = "<table><tr><th>Note</th><th>Related Chords</th></tr>", i=0, j = scale.length,k = 0, l = BuildScale.ChordsVO[chorder[i]].length;
		for(i; i < j; i+=1){
			html += '<tr';
			if(i === 0){	
				//root note
	...
	updateFretboard = function(root, scale){
		...
		var o = BuildScale.Build(
					root, 
					BuildScale.ModesVO[Number(scale)]
		);
	...
	searchScales = function(input, preventAddToHistory){
		...
		var arrInput = input.split(" "), root=0, scale=0, i=0,j=0,regex = new RegExp(arrInput[i], "i"), k=0, history = $("#searchHistory");
		
		for(i; i < arrInput.length; i+=1){
			if(arrInput[i].length === 1){
				//probably the root note
				for(j; j < BuildScale.ChromaticScaleVO.length; j+=1){
				...
				//console.log(regex);
				for(k; k < BuildScale.ModesVO.length; k+=1){
