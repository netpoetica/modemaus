$(function(){
	...
	var showOutput = function(rootNote, scaleName, scale, chorder){
		...
		for(var i=0,j = scale.length; i < j; i++){
			...
			//console.log(chords[chorder[i]]);
			
			for(k = 0, l = BuildScale.ChordsVO[chorder[i]].length; k < l; k++){
			 	//console.log(chords[chorder[i]][k]);
	...
	var updateFretboard = function(root, scale){
	...
	//////
	var searchScales = function(input, preventAddToHistory){
		if(!input){
			input = $("#txtSearch").val();
		}
